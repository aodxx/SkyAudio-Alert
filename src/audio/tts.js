// src/audio/tts.js
// Thai TTS adapter. Gemini is the production default; Edge/Google remain supported.
// Gemini is tuned for a warm village loudspeaker style rather than a newsreader.
...
async function synthesizeWithGemini(script, config, opts = {}) {
  ...
  if (wav.length < 44 || wav.subarray(0, 4).toString() !== 'RIFF') throw makeError('Gemini TTS returned audio without a valid WAV header');
  ...
  try {
    ...
    execFileSync('ffmpeg', ['-y', '-loglevel', 'error', '-i', wavPath, '-codec:a', 'libmp3lame', '-b:a', '128k', '-ar', '24000', '-ac', '1', mp3Path], { stdio: 'pipe', timeout: 120000 });
